...
public class SeparateChainingHashST<Key, Value> implements SymbolTable<Key, Value> {
    ...
    /*
     * An array of linked list nodes is the hash table. Each <Key> will hash to
     * a number which will be used as index into this array. If multiple <Key>s
     * hash to the same value, append a new Node to the tail of the Node.
     */
    private Node[] st = (Node[]) new Object[M]; // generic array creation is not
						// allowed, so the ugly cast.
    ...
    /*delete a linked list node given reference to that node*/
